...
class TestAzureOCRService(unittest.TestCase):
    ...
    def test_azure_ocr_service_on_folder(self):
        ...
        if self.ocr_images_folder is None or not os.path.isdir(self.ocr_images_folder):
            self.logger.error(
                f"OCR folder not found or invalid: {self.ocr_images_folder}"
            )
            ...
            while retries <= self.max_retries:
                try:
                    ...
                    if e.status_code == 429:
                        ...
                        self.logger.warning(
                            f"429 Too Many Requests received. Retry {retries + 1}/{self.max_retries} after {wait_time} seconds."
                        )
        ...
        self.logger.info(
            f"Processed {len(results)} images. OCR text saved to {output_csv_path}"
        )
